 ...
 *  애너테이션 타입 정의하기
 * 		@interface 애너테이션 이름 {
 * 		타입 요소이름(); // 반환값이 있고 매개변수는 없는 추상메서드 형태
 * 			...
 * 	}
 * 
 *  애너테이션 요소의 규칙
 *  	1. 요소의 타입은 기본형, Stringm, enum, annotation, class만 허용된다.
 *  	2. ()안에 매개변수를 선언할 수 없다.
 *  	3. 예외를 선언할 수 없다.
...
@Retention(RetentionPolicy.RUNTIME) // 유지되는 기간 (SOURCE, CLASS, RUNTIME) default : class
