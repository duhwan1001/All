...
public class T05_ServletCookieTest extends HttpServlet {
	/**
	 * 쿠키 정보를 다루기 위한 예제
	 * (웹서버와 브라우저는애플리케이션을 사용하는 동안 필요한 값을 쿠키를
	 * 통해 공유하여 상태를 유지함
	 * 1. 구성 요소
	 * - 이름
	 * - 값
	 * - 유효시간(초)
	 * -도메인 (ex) www.someohost.com, .somehost.com
	 * => 쿠키의 도메인이 쿠키를 생성한 섭의 도메인을 벗어나면 브라우저는 쿠키를 저장(생성)하지 않는다. (보안문제때문에)
	 * 
	 *  2. 동작방식
	 *  - 쿠키 생성단계 : 생성한 쿠리를 응답데이터의 헤더에 저장하여 엡브라우저에 전송한다.
	 *  - 쿠키 저장단계 : 브라우저는 응답ㅇ데이터의 헤더에 저장하여 웹브라우저에 전송한다.
	...
	private void readCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(cookies != null) {
			...
			for(int i=0; i<cookies.length; i++) {
				...
				out.println("value : " + URLDecoder.decode(
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		Cookie name = new Cookie("name", URLEncoder.encode(req.getParameter("name"),"utf-8")); // 반대는 decoder
