...
public class UploadServlet2 extends HttpServlet {
	...
	private static final int MEMORY_THRESHOLD = 1024 * 1024 * 3; // 3mb 언저리

	// 파일 1개당 최대 크기 // 메모리에 한계점
	private static final long MAX_FILE_SIZE = 1024 * 1024 * 40;
	
	// 요청 파일 최대 크기
	private static final long MAX_REQUEST_SIZE = 1024 * 1024 * 50; 
	// 3mb 이상은 임시파일로 저장되는데 한번에 올릴수있는 최대크기를 저장해주지 않으면 감당할수가 없음
					...
					for (FileItem item : formItems) {
						if (!item.isFormField()) { // 파일인 경우
							...
							formMap.put(item.getFieldName(), 
										item.getString("UTF-8")); // 가져오면서 UTF-8 로 인코딩 처리함
						}
					}
				}

			} catch (Exception e) {
				req.setAttribute("meessage", "예외발생: " + e.getMessage());
				e.printStackTrace();
			}
